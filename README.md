...
1. **多维度交互式绘图**：
   ...
   * ![DataPlot：轻量级数据可视化与格式转换工具](https://raw.githubusercontent.com/Wolframscs/PIC-Vault/main/pic/20260624211737529.png)
   * 支持鼠标缩放、拖拽及坐标数据悬停提示（Matplotlib 自带交互）。
   * 预设过滤选项，支持按“循环（Cycle）”和“工步（Step）”对海量数据进行快速过滤重绘。
   * ![DataPlot：轻量级数据可视化与格式转换工具-2](https://raw.githubusercontent.com/Wolframscs/PIC-Vault/main/pic/20260624211737530.png)
2. **高 DPI 与 4K 屏幕自适应**：
...
我们提供了预设的 PyInstaller 打包脚本 `DataPlot_EXE.py`，它会自动执行清理、排除冗余库并导出为 `--onedir` 格式：

```bash
python DataPlot_EXE.py
```
...
* `DataPlot.py`：绘图与分析主程序的 GUI 源码。
* `DataPlot_EXE.py`：PyInstaller 自动打包配置脚本。
